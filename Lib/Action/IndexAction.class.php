...
class IndexAction extends GlobalAction {
	...
	public function test1(){
		//查询所有用户
		$Ulist = M('user')->where()->select();
		foreach ($Ulist as $k=>$v){
			$data['userid'] = $Ulist[$k]['uid'];
			$data['uemail'] = $Ulist[$k]['email'];
			print_r($data['uemail']);
			M('friend')->add($data);
		}
// 		print_r($Ulist);
	...
	public function test(){
		$t_ulist = M('user')->where()->select();
		
		foreach($t_ulist as $k=>$v){

		$urModel = M('userrelationship');
    	$uphone = $t_ulist[$k]['phonenumber']; //当前登录用户手机号
    	$uname = $t_ulist[$k]['uname'];//当前登录用户姓名
    	$uemail = $t_ulist[$k]['email'];//当前登录用户账号
    	$uinfo = $this->getUinfo();//获取当前登录用户信息
    	$pagenum = (int)$_POST['pagenum'];//当前页数
    	$phone = $_POST['phone'];
	    	...
	    	//当手机端请求时则查询全部
	    	if($phone == "1"){
	    		$urList = $urModel->where($map)->order('id asc')->select();
	    	}else{
	    		$urList = $urModel->where($map)->order('id asc')->select();
	    	}
	    	...
	    	if($urList != null){
	    		foreach($urList as $k=>$v){
	    			if($urList[$k]['uemail1'] == $uemail){
	    				...
	    				$urList[$k]['time'] = $urList[$k]['time'];  //////////////////////////////
	    				
	    				$mapfff['uemail'] = $uemail;
	    				$ftlist = M('friend')->where($mapfff)->find();
	    				
	    				
						if($ftlist['friendemail']!=null){
	    					$data['friendemail'] = $ftlist['friendemail'].','.$urList[$k]['uemail1'];
						}else{
							$data['friendemail'] = $urList[$k]['uemail1'];
						}
						if($ftlist['friendemail']!=null){
							$data['time'] = $ftlist['time'].','.$urList[$k]['time'];
						}else{
							$data['time'] = $urList[$k]['time'];
						}
	    				
	    				$mapff['uemail'] = $uemail;
	    				M('friend')->where($mapff)->save($data);
	    			}else{
	    				$mapfff['uemail'] = $uemail;
	    				$ftlist = M('friend')->where($mapfff)->find();
...
//	    			}
    	...
    	$dataInfo['pagenum'] = $_POST['pagenum'];
...
//     			$result3 = $userModel->where($map)->save($data);
     ...
     * @author wangkai
     */
    public function queryMyFriend2(){
		...
		if($_POST['condition'] != null){//好友查询条件赋值
    		...
    		$mapfi['uname|phonenumber'] = array("like","%".$condition."%");
    	}

		//查询当前登录用户所有好友emial
		$mapU['uemail'] = $uemail;
		$fList = $fModel->where($mapU)->find();
    	...
    	if(preg_match($matchCircleid,$pagenum)){
	    	...
	    	//当手机端请求时则查询全部
			$femail = explode(',',$fList['friendemail']);
			$mapfi['email'] = array('in',$femail);
			$count = $uModel->where($mapfi)->count();// 查询满足要求的总记录数
	    	if($phone == "1"){
	    		$fInfoList = $uModel->where($mapfi)->order('uid asc')->select();
	    	}else{
	    		$fInfoList = $uModel->limit($pageFirst,$pagecount)->where($mapfi)->order('uid asc')->select();
	    	}
	    	
	    	$pagenumCount = ceil($count/$pagecount);//总页数

	    	if($pagenum < $pagenumCount and $pagenum != -2){//若当前页数比总页数小 则显示下一页按钮
	    		$dataInfo['pagenext'] = '1';
	    	}else{
	    		$dataInfo['pagenext'] = '-1';
	    	}
		...
		if($fInfoList!=null){
			foreach($fInfoList as $k=>$v){
					$fInfoList[$k]['uemail1'] = $fInfoList[$k]['email'];
		    		$fInfoList[$k]['uname1'] = $fInfoList[$k]['uname'];
		    		$fInfoList[$k]['uphonenumber1'] = $fInfoList[$k]['phonenumber'];
		    		$fInfoList[$k]['faceimage1'] = $fInfoList[$k]['faceimage'];
		    		$fInfoList[$k]['areacode1'] = $fInfoList[$k]['areacode'];
			}
			$dataInfo['info'] = $fInfoList;
		}
    ...
    public function queryMyFriend(){
    	...
    	if(preg_match($matchCircleid,$pagenum)){
	    	...
	    	if($phone == "1"){
	    		...
	    		$urList = $urModel->limit($pageFirst,$pagecount)->where($map)->order('id asc')->select();
	    	}
	    	$pagenumCount = ceil($count/$pagecount);//总页数
...
//	    			}
    ...
    public function getNewMsgNum(){
    	...
    	//查询圈子新申请信息统计
    	$caModel = M("circleapply");
    	$mapCa['uemail1'] = $uemail;
    	$mapCa['msgreadstatus'] = '0';//还未被阅读过的信息
    	$resultCaNum = $caModel->where($mapCa)->count();

    	if($resultFaNum != null or $resultCaNum != null){
    		$dataInFo['info'] = $resultFaNum + $resultCaNum;//新申请数目
    	}
    	$this->ajaxReturn($dataInFo,'JSON');
    }
    ...
    public function upAppMsgStatus(){
    	...
    	if($msgreadstatus == '1'){
    		...
    		//将圈子加入申请信息状态为已查看
    		$caModel->where($mapFa)->save($dataFa);
    	}
    	...
    	//查询头五条未处理申请加入圈子信息
    	$caList = $caModel->limit(5)->where($mapFa2)->select();
    	if($caList != null){
    		$dataInfo['info2'] = $caList;
    		$dataInfo['status'] = '2';//查询成功
    	}

    	if($faList == null and $caList == null){
    		...
    		if($resultIsCount >= 2){//当存在两条记录说明申请人和被申请人在同一个圈子当中
    			...
    			if($resultIsFa != null && $resultIsFa['status'] == '2'){
    				...
    				if($resultAdd != false){
    					...
    					//申请成功时发送邮件通知被申请好友
    					$email = $uemail2;
    					$head = "Hello! ".$uname."刚申请和你成为好友";
    					$body = "快前往 http://www.Hello008.com 确认吧！ 好吧，我也承认最讨厌这种邮件提醒什么的了，不过你朋友找你，总得提示你下吧，哈哈！";
    					$this->sendEmail($email,$head,$body);
    				}else{
     ...
     * @author wangkai
     */
    public function applyCircleCancel(){
    	 $uEmailApp = trim($_POST['emailapp']);//申请加入圈子的用户email
    	 $circleId = trim($_POST['circleid']);//要加入的圈子id

		$matchEmail = "/^[_.0-9a-z-a-z-]+@([0-9a-z][0-9a-z-]+.)+[a-z]{2,4}$/"; //email正则表达式

    	//email通过正则表达式验证
    	if(preg_match($matchEmail,$uEmailApp)){
    		$caModel = M('circleapply');//好友申请 信息表
    		$map['joincircleid'] = $circleId;
    		$map['uemail2'] = $uEmailApp;
    		$resultUf = $caModel->where($map)->delete();

    		if($resultUf != false){
    			$dataInfo['status'] = '1';//删除记录成功
    		}else{
    			$dataInfo['status'] = '-1';//删除记录失败
    		}
    ...
    public function applyAgree_2(){
    	...
    	if(preg_match($matchEmail,$uemail1)){
    		...
    		//添加好友
    		//查询是否已存在当前好友关系记录
    		$mapf1['uemail'] = $uemail1;
    		$mapf2['uemail'] = $uemail2;
			$fu1 = $fModel->where($mapf1)->find();
			$fu2 = $fModel->where($mapf2)->find();
			if($fu1 != null && $fu2 != null){//添加好友
				//判断是否已互为好友
				if(in_array($uemail2,$fu1['friendemail'])){
					$dataInfo['status'] = '1';//添加好友记录成功
				}else{
					$fu1['friendemail'] = $fu1['friendemail'].$uemail2.',';
					$fu1['time'] = $fu1['time'].time().',';
					$datafu1['friendemail'] = $fu1['friendemail'];
					$datafu1['time'] = $fu1['time'];

					$sr1 = $fModel->where($mapf1)->save($datafu1);
					$sr2 = $fModel->where()->save();
				...
				$datafu1['uemail'] = $uemail1.',';
				$datafu1['friendemail'] = $uemail2.',';
				$datafu1['time'] = time().',';

				$datafu2['uemail'] = $uemail2.',';
				$datafu2['friendemail'] = $uemail1.',';
				$datafu2['time'] = time().',';
				$dr1 = $fModel->add($datafu1);
				$dr2 = $fModel->add($datafu2);

				if($dr1!=false && $dr2!=false){
						$dataInfo['status'] = '1';//添加好友记录成功
				}else{
						$dataInfo['status'] = '-1';//添加好友记录失败
				}
...
//    					where($mapAf)->getField('status');
     ...
     * @author wangkai
     */
    public function applyCircleJoin(){
		$groupModel = M('group');
    	$uemail = $this->getUserName();//当前登录用户Email
    	$uinfo = $this->getUinfo();//获取当前用户的信息
		$circleId = $_POST['circleId'];//申请加入的圈子id
		$grModel = M('group');

		//验证圈子是否存在
		$mapgid['id'] = $circleId;
		$gresult = $groupModel->where($mapgid)->find();
		if($gresult == null or $gresult == false){
			$dataInfo['info'] = "-1";//该圈子id错误或者不存在
			$this->ajaxReturn($dataInfo,'JSON');
		}
    	 ...
    	 if($resultIsJo == null && $resultJoCount >= 8){
    	 	$dataInfo['info'] = '-200';//加入圈子数量暂时不能超过8个
    	 	$this->ajaxReturn($dataInfo,'JSON');
    	 }

		$uemail1 = $gresult['createuser'];//圈子创始人email

		//查询被申请圈子
		$data['uemail1'] = $uemail1;//被申请圈子创始人email账号
		$data['uemail2'] = $uemail;//申请人email账号
		$data['uname2'] = $uinfo['uname'];//申请人姓名
		$data['ufaceimg2'] = $uinfo['faceimage'];//申请者头像
		$data['info'] = '';//申请备注
		$data['status'] = '0';//申请信息处理状态（0,代表未处理.1,申请通过2,申请未通过）
		$data['msgreadstatus'] = '0';//信息查看状态（0，代表未被查看，1，代表已被查看）
		$data['joincircleid'] = $circleId;//申请圈子id
		$data['joincirclename'] = $gresult['name'];//申请圈子名称
		$data['time'] = time();//申请时间

		//插入申请
		$cappModel = M('circleapply');
		$aresult = $cappModel->add($data);
		if($aresult != false){
			$dataInfo['info'] = "1";//加入申请提交成功
			//申请成功时发送邮件通知创建者
    		$email = $uemail1;
    		$head = "Hello! ".$uinfo['uname']."刚申请加入你所创建的圈子'".$gresult['name']."'";
    		$body = "快前往 http://www.Hello008.com 确认吧！";
    		$this->sendEmail($email,$head,$body);
		}else{
			$dataInfo['info'] = "-2";//加入申请提交失败
		}
     ...
     * @author wangkai
     */
    public function applyCircleAgree(){
    	 //当用户加入非密码圈子时
    	 $uEmailApp = trim($_POST['emailapp']);//申请加入圈子的用户email
    	 $circleId = trim($_POST['circleid']);//要加入的圈子id
    	 ...
    	 //当申请者不为空时，检测通过者是否为圈子创建人
    	 if($uEmailApp != null && $uEmailApp != ''){
			$mapGroup['id'] = $circleId;
			$mapGroup['createuser'] = $uEmail;
			$resultIsC = $circleModel->where($mapGroup)->find();
			if($resultIsC == null || $resultIsC == false){
				$dataInfo['status'] = '-300';//不是圈子创始人
				$this->ajaxReturn($dataInfo,'JSON');
			}

			//查询申请加入者的信息
			$userModel = M('user');
			$mapU['email'] = $uEmailApp;
			$userInfo = $userModel->where($mapU)->find();

			//获取当前需要加入圈子用户信息
			$data['circleid'] = $circleId;
    	 	$data['uemail'] = $uEmailApp;
    	 	$data['time'] = time()."";
			...
			$result = $grModel->add($data);
    	 	...
    	 	if($result != false){
    	 		...
    	 		$dataInfo['status'] = '1';//加入圈子成功返回1
    	 		//删除当前申请记录
    	 		$mapdele['uemail2'] = $uEmailApp;
    	 		$mapdele['joincircleid'] = $circleId;
    	 		$cappModel->where($mapdele)->delete();

    	 		//申请成功时发送邮件通知创建者
    			$email = $uEmailApp;
    			$head = "Hello! 你申请加入圈子，已被对方通过了。";
    			$body = "快前往 http://www.Hello008.com 确认吧！";
    			$this->sendEmail($email,$head,$body);
    	 	}else{
    	 		$dataInfo['status'] = '-1';//加入圈子失败返回-1
    	 	}
...
//     	$map['id'] = $circleId;
    	...
    	if($circleName == null || $circleName == ''){$map['name|id'] =  array('notlike',"%11111111111111111111111111%");}
    		...
    		foreach($circleList as $k=>$v){
    			...
    			//查询是否申请加入该圈子
    			$appcirclModel = M('circleapply');
    			$mapApp['uemail2'] = $this->getUserName();//当前登录用户email
    			$mapApp['joincircleid'] = $circleId;//圈子id
    			$appRresult = $appcirclModel->where($mapApp)->find();

    			if($resultGr != null){
    				$circleList[$k]['isJo'] = 1;//是否已加入圈子标记 1代表已加入 2申请加入圈子 0代表未加入
    			}else if($appRresult != null && $appRresult != false){
					$circleList[$k]['isJo'] = 2;//已申请加入圈子
    			}else{
    				$circleList[$k]['isJo'] = 0;//是否已加入圈子标记0
    			}
    			$circleList[$k]['time'] = date("Y-m-d",$circleList[$k]['time']);
//     			$circleList[$k]['time'] = substr($circleList[$k]['time'],'2');
    		}
    		$data = $circleList;
    ...
    public function doImageUp(){
    	...
    	if($uEmail != null){
	    	...
	    	$upload->thumb = true;
	    	$upload->thumbMaxWidth = '50,400';
	    	$upload->thumbMaxHeight = '50,400';
